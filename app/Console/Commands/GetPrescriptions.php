...
class GetPrescriptions extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:GetPrescriptions';
     ...
     * @var string
     */
    protected $description = 'Get prescriptions of prescribed orders from liberty.';
            ...
            if (empty($order->liberty_patient_id)) {
                ...
                if ($response->ok()) {
                    ...
                    if (!empty($jsonResponse) && isset($jsonResponse[0])) {
                        ...
                        $prescriptionsResponse = $this->libertyAPI->getPatientPrescriptions($libertyPatientId, $startDate, $endDate);
                        $this->saveScriptNumber($order, $prescriptionsResponse);
                    }
                }

            } else {
                $prescriptionsResponse = $this->libertyAPI->getPatientPrescriptions($order->liberty_patient_id, $startDate, $endDate);
                $this->saveScriptNumber($order, $prescriptionsResponse);
            }
        }
    }
        ...
        if ($response->ok()) {
            ...
            if (!empty($scripts)) {
                foreach ($scripts as $scriptItem) {
                    if (
                        (float)$order->product_quantity === (float)$scriptItem->FullDispenseQuantity
                        && $productNdc === (int)$scriptItem->DrugPrescribed->NDC
                    ) {
                        ...
                        if(date('Y-m-d',strtotime($order->created_at))<=$scriptItem->WrittenDate && $scriptItem->LastRefillNumber==0){
                            ...
                            if ($orderRefill) {
                                $orderRefill->update([
                                    ...
                                    'refill_date' => $scriptItem->Fill->DispenseDate
                                ]);
                            } else {
