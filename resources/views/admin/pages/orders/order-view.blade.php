...
<li class="breadcrumb-item pe-3"><a href="{{URL::previous()}}" class="uri-redirect pe-3">{{ucwords($urlName[0])}}</a></li>
...
@if(in_array('admin.update.liberty.script.number',$permissions) || Auth::user()->u_type=='superadmin')
    @include('admin.pages.orders.modals.update-script-number-modal')
@endif

@if(in_array('admin.update.refill.ship.detail',$permissions) || Auth::user()->u_type=='superadmin')
    @include('admin.pages.orders.modals.mark-as-shipped-order-details-modal')
@endif    

@include('admin.pages.orders.modals.order-refill-modal')
@include('admin.pages.orders.modals.order-refill-tracking-modal')
...
@section('js')
    ...
    <script src="{{ asset('admin_assets/js/order-details.js') }}"></script>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', "#view-refill", function() {
            ...
            ajaxPostData(uri, '', 'GET', '#order-refill-data', 'orderRefillView', blockUIOrderRefillView,true,title,true)

        });
        $(document).on('click', "#view-refill-track", function() {
            let uri = $(this).attr('data-uri');
            var title = $(this).html();
            // $(this).addClass('loader-btn')
            ajaxPostData(uri, '', 'GET', '#order-refill-data', 'orderRefillView', blockUIOrderRefillView,true,title,true)
        ...
        $(document).on('click', "#view-refill-tracking", function() {
            let uri = $(this).attr('data-uri');
            var title = $(this).html();
            $(this).addClass('loader-btn')
            ajaxPostData(uri, '', 'GET', '#order-refill-tracking-data', 'orderRefillTrackingView', blockUIOrderRefillView,true,title,true)
