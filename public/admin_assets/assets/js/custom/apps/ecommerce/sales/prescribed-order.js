...
$(document).on("click", ".request_for_refill", function() {
    $.ajaxSetup({
      headers: {
    ...
    Swal.fire({
        title: 'Are you sure you want to request for refill?',
        icon: 'warning',
        showCancelButton: true,
...
$(document).on("click", ".reset-filter", function() {
    $("#flexRadioDefault").prop('checked',true);
    $("#kt_datatable_example_1").DataTable().ajax.reload();
});
...
$("#cancel-form").validate({
    ...
    submitHandler: function (form, event) { // for demo
        ...
        $.ajax({
            type: "POST",
            url: "/admin/cancel-subscription-by-admin",
            data: formData,
            dataType: "json",
            cache: false,
            contentType: false,
